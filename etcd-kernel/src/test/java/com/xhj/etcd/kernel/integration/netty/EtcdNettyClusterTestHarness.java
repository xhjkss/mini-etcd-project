...
/**
 * EtcdNettyClusterTestHarness
 *
 * @author XJks
 * @description Etcd + Raft + Netty RPC 多节点联调测试运行时。
 */
public class EtcdNettyClusterTestHarness {
    ...
    public String getValueViaRpc(String nodeId, String key) throws Exception {
        ...
        EtcdRpcResponse<GetResponse> response = EtcdNettyTestSupport.callGetByRpc(testClient, endpoint, key, false);
    ...
    public boolean isValueVisibleOnLeader(String key, String expected) throws Exception {
        ...
        EtcdRpcResponse<GetResponse> response = EtcdNettyTestSupport.callGetByRpc(testClient, leaderEndpoint, key);
        if (response == null || response.getHeader() == null || !response.getHeader().isSuccess() || response.getBody() == null) {
            return false;
        }
    ...
    private boolean isLeaderByRpc(NodeEndpoint endpoint) {
        try {
            EtcdRpcResponse<GetResponse> response = EtcdNettyTestSupport.callGetByRpc(testClient, endpoint, LEADER_PROBE_KEY);
    ...
    private class NodeRuntime {
        ...
        private void start() throws Exception {
            ...
            server.registerService(EtcdNode.RPC_SERVICE_NAME,
                    ...
                    EtcdNode.HANDLE_ETCD_RPC_LIST_KEYS_REQUEST_METHOD_NAME,
